...
(function () {
  ...
  function richText(s) {
    let out = esc(s);
    // Allow a safe subset of formatting tags the admin may type (bold / italic /
    // underline / line break). Everything else stays escaped (XSS-safe). This is
    // why <strong> works in every text field, not only the raw CMS blocks.
    out = out.replace(/&lt;(\/?)(strong|b|em|i|u)&gt;/gi, (m, sl, tag) => `<${sl}${tag.toLowerCase()}>`)
             .replace(/&lt;br\s*\/?&gt;/gi, '<br>');
